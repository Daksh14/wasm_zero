...
echo "  - wasm_zero's bindings.js is self-contained: the read path decodes"
echo "    straight from wasm memory with no runtime dependency. rkyv-js is"
echo "    imported only by modules that take a non-scalar argument (to encode"
echo "    it); the benchmark has none, so nothing extra ships."
